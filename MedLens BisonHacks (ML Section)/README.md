...
Do NOT commit API keys to source control. For local testing, you can set the env var in your shell.
...
Optional: use a `.env` file and a small loader like `dotenv` for convenience in development.

Run the test script
- Ensure you have a sample image at `./sample-discharge.png` (or update the path in `test-script.js`).
- Run:
```bash
node test-script.js
```
...
Next steps
- Add a lightweight `package.json` and a sample `.env.example` if you'd like.
